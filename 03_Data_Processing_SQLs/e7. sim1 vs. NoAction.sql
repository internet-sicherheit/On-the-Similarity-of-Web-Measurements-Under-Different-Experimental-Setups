-- This is the SQL used to generate the results when comparing profile Sim1 vs. Noaction.
  /*,
  0: openwpm_desktop,
...
SELECT
  ...
  WHERE
    freq.in_1>0) Sim1; /*,
  --e7.2 freq OF nodes sim1 vs passive1*/
SELECT
  (
  SELECT
    SUM(freq.in_0)
...
SELECT
  ...
  WHERE
    freq.in_0>0
    AND meta.is_third_party ) Passive1,
  (
  SELECT
    COUNT(*)
  FROM
